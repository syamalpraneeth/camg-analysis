...
def reassemble(h1, b1, h2, b2, h3, b3, h4, b4, h5, b5):  # rearrange all b entries
  un1 = union(b1, b2)
  un2 = union(b3, un1)
  un3 = union(b4, un2)
  un4 = union(b5, un3)
  ...
  ncdf = [x for x in un4 if x not in in4]
  ...
  return [H1, b, H2, b, H3, b, H4, b, H5, b];

def collate_ico(name, o1, o2, o3, o4, o5, p1, p2, p3, p4, p5, l1, l2, l3, l4, l5, cs, typ):
  if typ == 1:
    comm = 'As Prepared'
    tag = 'asp'
  elif typ == 2:
  ...
  i_ico = "<0 0 12 0>"
  a1 = [o1.a[o1.b.index(i_ico)], o2.a[o2.b.index(i_ico)], o3.a[o3.b.index(i_ico)], o4.a[o4.b.index(i_ico)], o5.a[o5.b.index(i_ico)]]
  #a2 = [p1.a[p1.b.index(i_ico)], p2.a[p2.b.index(i_ico)], p3.a[p3.b.index(i_ico)], p4.a[p4.b.index(i_ico)], p5.a[p5.b.index(i_ico)]]
...
#  rows = zip(a1, a2, a3)
  ...
  b1 = [o1.h[0], o2.h[0], o3.h[0], o4.h[0], o5.h[0]]
  #b2 = [p1.h[0], p2.h[0], p3.h[0], p4.h[0], p5.h[0]]
  #rows2 = zip(b1, b2, a3)
  rows2 = zip(b1, a3)
  ...
  c1 = [sum(o1.h[1:]), sum(o2.h[1:]), sum(o3.h[1:]), sum(o4.h[1:]), sum(o5.h[1:])]
  #c2 = [sum(p1.h[1:]), sum(p2.h[1:]), sum(p3.h[1:]), sum(p4.h[1:]), sum(p5.h[1:])]
  #rows3 = zip(c1, c2, a3)
  rows3 = zip(c1, a3)
...
def ovito_strain(name, pipeline, fil, typ, case):
  ...
  file='/home/mj0054/Documents/work/simulations/projects/nanoglass/50-50/1e10/segregated/random/'+name.split(' ')[-1]+'/comp_corrected/refconfig_'+case+'.txt'
  ...
  fil3 = 'tmp.avgstrain_coll_'+name.replace(" ", "_")+'_'+tag+'_All'
  with open(fil3, 'a') as f:
    # for row in rows:
    rows2 = str(sum(strain)/len(strain))+','+case
    f.write(rows2)
    f.write('\n')
  return []
