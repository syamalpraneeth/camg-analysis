...
def plot_allrdf(name, case1, case2, case3, case4, case5, cs, typ):
  if typ == 1:
    comm = 'As Prepared'
    tag = 'asp'
  elif typ == 2:
  ...
  for case in [case1, case2, case3, case4, case5]:
    ...
    i+=1
    p1 = axs[0].plot(x, y1+(6-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p2 = axs[1].plot(x, y2+(6-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p3 = axs[2].plot(x, y3+(6-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    ...
    for ax in axs:
      ax.text(0, 0.08+(6 - i)*w, case, fontsize='10')
...
def pe_plot_species(name, c1, c2, c3, c4, c5, cs, typ):
  if cs == 0:
    csstr = 'All'
  elif cs == 1:
    csstr = 'Core'
...
def voro_plot_species(name, m1, m2, m3, m4, m5, l1, l2, l3, l4, l5, species, cs, typ):
  voro_plot_sort(name, m1, m2, m3, m4, m5, l1, l2, l3, l4, l5, species, cs, typ)
  h1, b1 = m1.a, m1.b
  h2, b2 = m2.a, m2.b
  h3, b3 = m3.a, m3.b
  h4, b4 = m4.a, m4.b
  h5, b5 = m5.a, m5.b
  h1, b1, h2, b2, h3, b3, h4, b4, h5, b5 = reassemble(h1, b1, h2, b2, h3, b3, h4, b4, h5, b5)
  if species == 0:
    case = 'All'
...
def plot_atmhisto(name,m1, m2, m3, m4, m5, l1, l2, l3, l4, l5, species, cs, typ):
  plot_atmhisto_nico(name,m1, m2, m3, m4, m5, l1, l2, l3, l4, l5, species, cs, typ)
  h1, n1 = m1.c, m1.d
  h2, n2 = m2.c, m2.d
  h3, n3 = m3.c, m3.d
  h4, n4 = m4.c, m4.d
  ...
  y1=max(max(a1),max(a2),max(a3),max(a4),max(a5))
  y2=min(min(a1),min(a2),min(a3),min(a4),min(a5))
  x1=max(max(c1),max(c2),max(c3),max(c4),max(c5))
  x2=min(min(c1),min(c2),min(c3),min(c4),min(c5))
  #print('Max Y',yl)
  yl1=y1*1.05
  yl2=y2*0.90
  xl1 = x1 * 1.05
  ...
  tot_v = [v1.sum(axis=0), v2.sum(axis=0), v3.sum(axis=0), v4.sum(axis=0), v5.sum(axis=0)]
  ...
  if species == 0:
    ...
    rows2=zip(c1,a1,c2,a2,c3,a3,c4,a4,c5,a5,c6,a6)
    fil = 'tmp.atvolume_' + name.replace(" ", "_") + '_' + tag + '_' + csstr
    with open(fil, 'w') as f:
      writer = csv.writer(f)
      writer.writerow([l1,'Counts',l2,'Counts',l3,'Counts',l4,'Counts',l5,'Counts',l6,'Counts'])
      for row in rows2:
        writer.writerow(row)

def plot_chains(name, m1, m2, m3, m4, m5, l1, l2, l3, l4, l5, species, cs, typ):
  if species == 0:
    case = 'All'
  elif species == 1 or species == 3:
    case = 'Cu'
  ...
  # for i in range(0,maxch,iter):
    ...
    chain_tot = [len([y for y in x.csa if y <= i])*100/len(x.csa) for x in [m1, m2, m3, m4, m5]] #before it was x.d

    # chain_tot = [numpy.sum(x[i:]) for x in [y1,y2,y3,y4,y5,y6]]
    plt.plot([1,2,3,4,5],chain_tot,'k-',marker=markers[it],label=str(i),lw=1, zorder=3)
  ...
  plt.xticks([1, 2, 3, 4, 5], [l1, l2, l3, l4, l5], rotation=45)  # labelsize=15)
  ...
  # print(s3)
  itr = 1
  for i,j,k in zip([c1,c2,c3,c4,c5],[p1,p2,p3,p4,p5],[l1,l2,l3,l4,l5]):
  ...
  avgclus = [sum(x) / len(x) for x in [m1.csa, m2.csa, m3.csa, m4.csa, m5.csa]]
  ax.plot([1, 2, 3, 4, 5], avgclus, 'o:', c='black', mfc='blue',mec='k', markersize=9)
  ax.set_ylabel('Avg String Size', fontsize=16)
  ax.set_xticks([1, 2, 3, 4, 5])
  ax.set_xticklabels([l1, l2, l3, l4, l5], rotation=45, fontsize=14)
  ax.tick_params(axis="y", direction="in", bottom="on", top='on', labelsize=14)
  ...
  if species == 0:
    rows=zip(avgclus,[l1, l2, l3, l4, l5])
