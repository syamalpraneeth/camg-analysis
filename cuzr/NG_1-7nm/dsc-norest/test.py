...
src = '/home/mj0054/Documents/work/simulations/projects/nanoglass/50-50/1e10/segregated/random/5GPa/comp_corrected/dsc/'
# fn = 'log.file'
...
# for i in [2]:
  ...
  ax.plot(X, Y, label=str(i)+'nm NG') #, mec='k')
  axins1.plot(t,T)
...
a = [['T'+str(i)]+[str(i)] for i in sizes]
cols = []
for i1 in a:
  for i2 in i1:
    cols.append(str(i2))
...
# print(cols)
# print(numpy.shape(numpy.asarray(data)))
# print(numpy.array(data))
# print(data)
print(numpy.shape(numpy.transpose(numpy.array(data))))
enthalpies = pd.DataFrame(numpy.transpose(numpy.array(data)), columns=cols)
# enthalpies = pd.DataFrame(numpy.asarray(data), columns=cols)
# print(enthalpies)

enthalpies.to_csv('enth.csv')
