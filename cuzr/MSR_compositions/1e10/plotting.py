...
def plot_allrdf(name, case1, case2, case3, cs, typ):
  if typ == 1:
    comm = 'As Prepared'
    tag = 'asp'
  elif typ == 2:
  ...
  for case in [case1, case2, case3]:
    ...
    i+=1
    p1 = axs[0].plot(x, y1+(3-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p2 = axs[1].plot(x, y2+(3-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p3 = axs[2].plot(x, y3+(3-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
...
def pe_plot_species(name, c1, c2, c3, cs, typ):
  if cs == 0:
    csstr = 'All'
  elif cs == 1:
    csstr = 'Core'
  ...
  fig, (ax, ax2) = plt.subplots(1, 2, figsize=(8, 5), sharey=True)
  for axis in [ax,ax2]:
  ...
  ax.set_ylabel('Normalised Counts', fontsize=16)
  ...
  x1, x2, y1, y2 = -6.5, -6.3, 0.054, 0.078
  axins1.set_xlim(x1, x2)
  axins1.set_ylim(y1, y2)
  x1, x2, y1, y2 = -3.6, -3.4, 0.062, 0.115
  ...
  x1, y1 = [], []
  fil2 = open('tmp.pote_coll_CuZr_MG_asp_' + csstr, 'r')
  for line in fil2:
...
def voro_plot_species(name, m1, m2, m3, l1, l2, l3, species, cs, typ):
  voro_plot_sort(name, m1, m2, m3, l1, l2, l3, species, cs, typ)
  h1, b1 = m1.a, m1.b
  h2, b2 = m2.a, m2.b
  h3, b3 = m3.a, m3.b
  # h4, b4 = m4.a, m4.b
  h1, b1, h2, b2, h3, b3 = reassemble(h1, b1, h2, b2, h3, b3)
  if species == 0:
    case = 'All'
  ...
  if species == 0: ax.legend(fontsize=13, loc='upper right')
  else: ax.legend(fontsize=13, loc='lower right', title=case)
...
def plot_atmhisto(name,m1, m2, m3, l1, l2, l3, species, cs, typ):
  plot_atmhisto_nico(name,m1, m2, m3, l1, l2, l3, species, cs, typ)
  h1, n1 = m1.c, m1.d
  h2, n2 = m2.c, m2.d
  h3, n3 = m3.c, m3.d
  ...
  if species == 0:
    ...
    rows2=zip(c1,a1,c2,a2,c3,a3)
    fil = 'tmp.atvolume_' + name.replace(" ", "_") + '_' + tag + '_' + csstr
    with open(fil, 'w') as f:
      writer = csv.writer(f)
      writer.writerow([l1,'Counts',l2,'Counts',l3,'Counts'])
      for row in rows2:
        writer.writerow(row)

def plot_chains(name, m1, m2, m3, l1, l2, l3, species, cs, typ):
  if species == 0:
    case = 'All'
  elif species == 1 or species == 3:
    case = 'Cu'
  ...
  # for i in range(0,maxch,iter):
    ...
    chain_tot = [len([y for y in x.csa if y <= i])*100/len(x.csa) for x in [m1, m2, m3]] #before it was x.d

    # chain_tot = [numpy.sum(x[i:]) for x in [y1,y2,y3,y4,y5,y6]]
    plt.plot([1,2,3],chain_tot,'k-',marker=markers[it],label=str(i),lw=1, zorder=3)
  ...
  plt.xticks([1, 2, 3], [l1, l2, l3], rotation=45)  # labelsize=15)
  ...
  # print(s3)
  itr = 1
  for i,j,k in zip([c1,c2,c3],[p1,p2,p3],[l1,l2,l3]):
  ...
  avgclus = [sum(x) / len(x) for x in [m1.csa, m2.csa, m3.csa]]
  ax.plot([1, 2, 3], avgclus, 'o:', c='black', mfc='blue',mec='k', markersize=9)
  ax.set_ylabel('Avg String Size', fontsize=16)
  ax.set_xticks([1, 2, 3])
  ax.set_xticklabels([l1, l2, l3], rotation=45, fontsize=14)
  ax.tick_params(axis="y", direction="in", bottom="on", top='on', labelsize=14)
