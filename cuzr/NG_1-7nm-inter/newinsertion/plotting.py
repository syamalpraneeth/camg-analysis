...
def plot_rdf(name, case, delsub, cs, typ):
  ...
  i = 0
  x, y1, y2, y3 = [], [], [], []
  fil = open('tmp.rdf_' + case+'_'+csstr+'_'+tag, 'r')
  for line in fil:
  ...
  ax.legend(fontsize=12, title= name[-4:]+','+case)
...
def plot_allrdf(name, case1, case2, case3, case4, case5, case6, case7, case8, cs, typ):
  if typ == 1:
    comm = 'As Prepared'
    tag = 'asp'
  elif typ == 2:
  ...
  for case in [case1, case2, case3, case4, case5, case6, case7, case8]:
    ...
    i+=1
    p1 = axs[0].plot(x, y1+(8-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p2 = axs[1].plot(x, y2+(8-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    p3 = axs[2].plot(x, y3+(8-i)*w, label='_Hidden', color=cscheme(i), zorder=3, lw=2)
    ...
    for ax in axs:
      ax.text(0, 0.08+(8 - i)*w, case, fontsize='10')
  ...
  axs[0].legend(fontsize=14, title= name[-4:] + ', ' + 'Cu-Cu')
  axs[1].legend(fontsize=14, title= name[-4:] + ', ' + 'Cu-Zr')
  axs[2].legend(fontsize=14, title= name[-4:] + ', ' + 'Zr-Zr')
  # axs[0].legend(fontsize=12, title="{:.0e}".format(float(name.split(" ")[-1])) + r'$\frac{K}{s}$' + ',' + 'Cu-Cu')
  # axs[1].legend(fontsize=12, title="{:.0e}".format(float(name.split(" ")[-1])) + r'$\frac{K}{s}$' + ',' + 'Cu-Zr')
  # axs[2].legend(fontsize=12, title="{:.0e}".format(float(name.split(" ")[-1])) + r'$\frac{K}{s}$' + ',' + 'Zr-Zr')

  plt.savefig('rdf_' + name.replace(" ", "_") +  '_' + tag + '_' + csstr + '.png', dpi=400)
  plt.close()

def pe_plot_species(name, c1, c2, c3, c4, c5, cs, c6, c7, c8, typ):
  if cs == 0:
    csstr = 'All'
  elif cs == 1:
    csstr = 'Core'
  ...
  i = 0
  rng = [c1, c2, c3, c4, c5, c6, c7, c8]
  for c in rng:
    x1, y1 = [], []
    if cs == 0: fil = open('tmp.histo_' + c + '_' + tag, 'r')
    if cs > 0: fil = open('tmp.histo_' + c + '_' + tag + '_' + csstr, 'r')
    next(fil)
    next(fil)
    for line in fil:
      values = [float(s) for s in line.split()]
  ...
  for axis in [ax,ax2]:
    ...
    p5 = axis.plot(x[:, 4], y_norm[:, 4], color=cscheme(5), label=c5,zorder=3, lw=2)
    p6 = axis.plot(x[:, 4], y_norm[:, 4], color=cscheme(6), label=c6,zorder=3, lw=2)
    p7 = axis.plot(x[:, 4], y_norm[:, 4], color=cscheme(7), label=c7,zorder=3, lw=2)
    p8 = axis.plot(x[:, 4], y_norm[:, 4], color=cscheme(8), label=c8,zorder=3, lw=2)

  qr = name.split(" ")[-1].split("e")
  ax.legend(fontsize=12, bbox_to_anchor=(1, 0.6), loc='upper right', title= name[-4:])
  ...
  for axis in [axins1,axins2]:
    ...
    axis.plot(x[:, 6], y_norm[:, 6], color=cscheme(7), label=c7)
    axis.plot(x[:, 7], y_norm[:, 7], color=cscheme(8), label=c8)
...
def voro_plot_species(name, m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8, species, cs, typ):
  voro_plot_sort(name, m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8, species, cs, typ)
  h1, b1 = m1.a, m1.b
  h2, b2 = m2.a, m2.b
  h3, b3 = m3.a, m3.b
  h4, b4 = m4.a, m4.b
  ...
  h1, b1, h2, b2, h3, b3, h4, b4, h5, b5, h6, b6, h7, b7, h8, b8 = reassemble(h1, b1, h2, b2, h3, b3, h4, b4, h5, b5, h6, b6, h7, b7, h8, b8)
  if species == 0:
    case = 'All'
  ...
  p5 = ax.bar(y1 + 4 * w, h5, width=0.15, label=l5, color=cscheme(5), edgecolor='black', hatch="o",zorder=3)
  p6 = ax.bar(y1 + 5 * w, h6, width=0.15, label=l6, color=cscheme(6), edgecolor='black', hatch="/",zorder=3)
  p7 = ax.bar(y1 + 6 * w, h7, width=0.15, label=l7, color=cscheme(7), edgecolor='black', hatch="*",zorder=3)
  p8 = ax.bar(y1 + 7 * w, h8, width=0.15, label=l8, color=cscheme(8), edgecolor='black', hatch="|",zorder=3)

  plt.xticks(y1, b1, rotation=90)

  qr = name.split(" ")[-1].split("e") #bbox_to_anchor=(1, 0.6),
  if species == 0: ax.legend(fontsize=13, loc=1, title=name[-4:])
  else: ax.legend(fontsize=13, loc=1, title=name[-4:] + r', ' + case)
  ...
  #    ax.invert_yaxis()
  yl = max(max(h1, h2, h3, h4, h5)) * 1.05
  ax.set_ylim(0, yl)
...
def plot_atmhisto(name,m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8, species, cs, typ):
  plot_atmhisto_nico(name,m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8, species, cs, typ)
  h1, n1 = m1.c, m1.d
  h2, n2 = m2.c, m2.d
  h3, n3 = m3.c, m3.d
  h4, n4 = m4.c, m4.d
  ...
  bin5 = math.floor(n5 / 50)
  bin6 = math.floor(n6 / 50)
  bin7 = math.floor(n7 / 50)
  bin8 = math.floor(n8 / 50)
  ...
  ht5, b5 = numpy.histogram(h5, bins=bin1)
  ht6, b6 = numpy.histogram(h6, bins=bin1)
  ht7, b7 = numpy.histogram(h7, bins=bin1)
  ht8, b8 = numpy.histogram(h8, bins=bin1)

  a1, c1 = ht1 / n1, (b1[:-1] + b1[1:]) / 2
  a2, c2 = ht2 / n2, (b2[:-1] + b2[1:]) / 2
  a3, c3 = ht3 / n3, (b3[:-1] + b3[1:]) / 2
  a4, c4 = ht4 / n4, (b4[:-1] + b4[1:]) / 2
  a5, c5 = ht5 / n5, (b5[:-1] + b5[1:]) / 2
  a6, c6 = ht6 / n6, (b6[:-1] + b6[1:]) / 2
  a7, c7 = ht7 / n7, (b7[:-1] + b7[1:]) / 2
  a8, c8 = ht8 / n8, (b8[:-1] + b8[1:]) / 2
  ...
  plt.plot(c5, a5, color=cscheme(5), label=l5,zorder=3,lw=2)
  plt.plot(c6, a6, color=cscheme(6), label=l6,zorder=3,lw=2)
  plt.plot(c7, a7, color=cscheme(7), label=l7,zorder=3,lw=2)
  plt.plot(c8, a8, color=cscheme(8), label=l8,zorder=3,lw=2)
  ...
  y1=max(max(a1),max(a2),max(a3),max(a4),max(a5),max(a6),max(a7),max(a8))
  y2=min(min(a1),min(a2),min(a3),min(a4),min(a5),min(a6),min(a7),min(a8))
  x1=max(max(c1),max(c2),max(c3),max(c4),max(c5),max(c6),max(c7),max(c8))
  x2=min(min(c1),min(c2),min(c3),min(c4),min(c5),min(c6),min(c7),min(c8))
  #print('Max Y',yl)
  yl1=y1*1.05
  yl2=y2*0.90
  xl1 = x1 * 1.05
  ...
  tot_v = [v1.sum(axis=0), v2.sum(axis=0), v3.sum(axis=0), v4.sum(axis=0), v5.sum(axis=0), v6.sum(axis=0), v7.sum(axis=0), v8.sum(axis=0)]
  ...
  if species == 0:
    ...
    rows2=zip(c1,a1,c2,a2,c3,a3,c4,a4,c5,a5,c6,a6,c7,a7,c8,a8)
    fil = 'tmp.atvolume_' + name.replace(" ", "_") + '_' + tag + '_' + csstr
    with open(fil, 'w') as f:
      writer = csv.writer(f)
      writer.writerow([l1,'Counts',l2,'Counts',l3,'Counts',l4,'Counts',l5,'Counts',l6,'Counts',l7,'Counts',l8,'Counts'])
      for row in rows2:
        writer.writerow(row)

def plot_chains(name, m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8, species, cs, typ):
  if species == 0:
    case = 'All'
  elif species == 1 or species == 3:
    case = 'Cu'
  ...
  ht5, b5 = numpy.histogram(m5.csa, max(m5.csa))
  ht6, b6 = numpy.histogram(m6.csa, max(m6.csa))
  ht7, b7 = numpy.histogram(m7.csa, max(m7.csa))
  ht8, b8 = numpy.histogram(m8.csa, max(m8.csa))
  ...
  y5, c5 = ht5 / sum(m5.csa), [x for x in range(1,len(b5))]
  y6, c6 = ht6 / sum(m6.csa), [x for x in range(1,len(b6))]
  y7, c7 = ht7 / sum(m7.csa), [x for x in range(1,len(b7))]
  y8, c8 = ht8 / sum(m8.csa), [x for x in range(1,len(b8))]
  ...
  # for i in range(0,maxch,iter):
    ...
    chain_tot = [len([y for y in x.csa if y <= i])*100/len(x.csa) for x in [m1, m2, m3, m4, m5, m6, m7, m8]] #before it was x.d

    # chain_tot = [numpy.sum(x[i:]) for x in [y1,y2,y3,y4,y5,y6]]
    plt.plot([1,2,3,4,5,6,7,8],chain_tot,'k-',marker=markers[it],label=str(i),lw=1, zorder=3)
  ...
  plt.xticks([1, 2, 3, 4, 5, 6, 7, 8], [l1, l2, l3, l4, l5, l6, l7, l8], rotation=45)  # labelsize=15)
  ...
  p5 = [x * y *100 for x, y in zip(c5, y5)]
  p6 = [x * y *100 for x, y in zip(c6, y6)]
  p7 = [x * y *100 for x, y in zip(c7, y7)]
  p8 = [x * y *100 for x, y in zip(c8, y8)]
  ...
  # print(s3)
  itr = 1
  for i,j,k in zip([c1,c2,c3,c4,c5,c6,c7,c8],[p1,p2,p3,p4,p5,p6,p7,p8],[l1,l2,l3,l4,l5,l6,l7,l8]):
  ...
  avgclus = [sum(x) / len(x) for x in [m1.csa, m2.csa, m3.csa, m4.csa, m5.csa, m6.csa, m7.csa, m8.csa]]
  ax.plot([1, 2, 3, 4, 5, 6, 7, 8], avgclus, 'o:', c='black', mfc='blue',mec='k', markersize=9)
  ax.set_ylabel('Avg String Size', fontsize=16)
  ax.set_xticks([1, 2, 3, 4, 5, 6, 7, 8])
  ax.set_xticklabels([l1, l2, l3, l4, l5, l6, l7, l8], rotation=45, fontsize=14)
  ax.tick_params(axis="y", direction="in", bottom="on", top='on', labelsize=14)
  ...
  if species == 0:
    rows=zip(avgclus,[l1, l2, l3, l4, l5, l6, l7, l8])
    fil = 'tmp.avg-chains_' + name.replace(" ", "_") + '_' + tag + '_' + csstr
    with open(fil, 'w') as f:
      writer = csv.writer(f)
      for row in rows:
        writer.writerow(row)