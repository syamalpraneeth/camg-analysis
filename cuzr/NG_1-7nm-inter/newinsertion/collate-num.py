...
casedict = {"2GPa":"darkorange", "5GPa":"red"}
...
def plot_collated(source,ylab,c,name,tag,only):
    ...
    for cs in csval:
      for t in tag:
        with open(m+'/tmp.'+source+'_coll_'+name.replace(" ", "_")+'_'+m+'_'+str(t)+'_All','r') as csvfile:
          ...
          l = []
          for row in plots:
            if source=='strain': x.append(float(row[1]))
        ...
        # x.pop(0)
        nl = [int(i) for i in l]
        nx = [float(i) for i in x]
        if source == 'pote':
          #print(x,m,s,cs,t)
          ax.plot(nl, [-4.9728 for j in range(0,len(nl))], '--', color='gray',zorder=5) #x = [i/(-4.979) for i in x]
          ax.text(30, -4.9705, 'MG As Prepared', color='gray', fontsize='10')
        if t == tag[0]: ax.plot(nl, nx, 'o:', c='black', mfc=casedict[m],mec='k',  label=m+' '+tagdict[t],ms=9,zorder=6)
        else: ax.plot(nl, nx, 's:', c='black', mfc=casedict[m],mec='k',  label=m+' '+tagdict[t],ms=10,zorder=5)
        ax.set_xscale("log")
  ...
  plt.savefig('collated/num/'+source+'_coll_'+name.replace(" ", "_")+'.png',dpi=400)
  plt.clf()
  plt.close()
...
plot_collated('strain', r'Atomic readjustment (at % w. $\eta \geq 1.5$)', cases, name, tag, 1)
