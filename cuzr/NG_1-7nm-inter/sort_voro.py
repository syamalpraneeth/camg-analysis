...
def voro_plot_sort(name, m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8,species,cs,typ):
    h1 = m1.h
    h2 = m2.h
    h3 = m3.h
    h4 = m4.h
    ...
    for axs in [ax,ax2,ax3]:
      ...
      p5 = axs.bar(y1 + 4 * w, h5, width=0.2, label=l5, color=cscheme(5), edgecolor='black', hatch="o", zorder=3)
    ...
    if species == 0:
      ax3.legend(fontsize=12, title=name[-4:], loc=10,
                 bbox_to_anchor=(0.02,1.45,1,1))
    else:
      ax3.legend(fontsize=12, title=name[-4:] + ', ' + case + '',
                bbox_to_anchor=(0.02, 1.45, 1, 1))

    # yl = max(max(h1, h2, h3, h4, h5, h6))*1.10
    yl = (max(h1[3], h2[3], h3[3], h4[3], h5[3], h6[3], h7[3], h8[3])) * 1.05
    ylmin = (min(h1[3], h2[3], h3[3], h4[3], h5[3], h6[3], h7[3], h8[3])) * 0.97
    yl3 = (max(h1[2], h2[2], h3[2], h4[2], h5[2], h6[2], h7[2], h8[2])) * 1.10
    yl2 = (max(h1[0], h2[0], h3[0], h4[0], h5[0], h6[0], h7[0], h8[0])) * 1.05
    yl2min = (min(h1[0], h2[0], h3[0], h4[0], h5[0], h6[0], h7[0], h8[0])) * 0.95
...
def plot_atmhisto_nico(name,m1, m2, m3, m4, m5, m6, m7, m8, l1, l2, l3, l4, l5, l6, l7, l8,species,cs,typ):
    h1,n1 = m1.v,m1.d
    h2,n2 = m2.v,m2.d
    h3,n3 = m3.v,m3.d
    h4,n4 = m4.v,m4.d
    ...
    tot_v = [v1.sum(axis=0)/n1,v2.sum(axis=0)/n2,v3.sum(axis=0)/n3,v4.sum(axis=0)/n4,v5.sum(axis=0)/n5,v6.sum(axis=0)/n6,v7.sum(axis=0)/n7,v8.sum(axis=0)/n8]
