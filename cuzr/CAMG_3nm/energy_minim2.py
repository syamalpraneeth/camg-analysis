...
def minimize(name,src,f1,l1,f2,l2,f3,l3,f4,l4,f5,l5,f6,l6):
  ...
  #     next(fh)
    ...
    subloc = f1.replace(f2,'').split(".")[0][:-4]
    print(f1)
    print(f2)
    print(subloc)
    ...
    if "MG" in lab: l.command("write_dump all custom dump.minim."+lab+" id type x y z vx vy vz f_fpe")
    elif "NG" in lab: l.command("write_dump all custom dump.minim."+lab+" id type x y z vx vy vz i_int f_fpe")
    else: l.command("write_dump all custom dump.minim."+lab+" id type x y z vx vy vz i_int i_lyr f_fpe")
    l.command("clear")
    # l.close()
    pipeline = import_file("dump.minim."+lab)
    ...
    if numpy.max(atom_types)>2:
      ...
      data = pipeline.compute()
    pipeline = rem_surf(pipeline,lab)
    data = pipeline.compute()
