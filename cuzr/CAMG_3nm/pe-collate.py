...
def plot_collated(source,ylab,c,name,tag,only):
  ...
  for m in c:
    for s in siz:
      ...
      for cs in csval:
        ...
        # print(csdict(cs))
        if csind == 0: ax[i].plot(l, x, 'o',c='black', mfc=csdict(cs),mec='k',  label=cs,ms=10,zorder=5)
        if csind == 1: ax[i].plot(l, x, '^:', c='black' , mfc=csdict(cs), mec='k', label=cs,ms=8,zorder=3)
        if csind == 2: ax[i].plot(l, x, 's:', c='black', mfc=csdict(cs), mec='k', label=cs,ms=8,zorder=4)
        ...
        else: ax[i].tick_params(axis="y",direction="in",left='off',bottom='False',labelsize=14,colors='white')
        ...
        ax[i].tick_params(axis="x",direction="in",left='off',bottom='False',labelsize=14,colors='white')
        ...
        if i ==0 :
          if source != 'vol' and source != 'nicovol':
            ...
            else: plt.legend(loc='upper left',fontsize=13)
  ...
  for m,a in zip(c,ax):
    ...
    # if cs == 'All':
    if source == 'ico':
      a.text(0.7, 0.75, r'${10}^{' + str(qr[1]) + '}$' + ' K/s',transform=a.transAxes, fontsize='15')
    else:
      a.text(0.7, 0.65, r'${10}^{' + str(qr[1]) + '}$' + ' K/s',transform=a.transAxes, fontsize='15')

  fig.tight_layout(rect=[0, 0, 1, 0.95])
  plt.savefig('collated/'+source+'-illus_coll_'+name.replace(" ", "_")+'.png',dpi=400)
  plt.clf()
  plt.close()
